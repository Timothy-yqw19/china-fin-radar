...
**一条命令抓齐四重视角**：官方原文（政策文件库 / 部委 / 央行）→ 权威媒体（新华社、
三大证券报、政府网要闻）→ 海外解读（路透/彭博等经 Google News 聚合、美联储、
世界银行、Rhodium Group）→ 专题分析（脉络 / 现状 / 各方动作 / 外部视角）。
抓完自动打分、出热词榜、生成日报与专题网页。

它同时是一个**秋招金融知识库**：每个热词配好定义、出处、考点清单、
60–90 秒口语化答法和追问应对，可直接刷题。
...
│   ├── feeds.py         外部视角（Google News / 美联储 / 世界银行 / Rhodium）
│   ├── views.py         权威媒体与外部视角的筛选 + 中英词表映射
